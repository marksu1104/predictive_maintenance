...
randomforestM=randomForest(hdf~AirTemperature_K+ProcessTemperature_K+RotationalSpeed_rpm+Torque_Nm+ToolWear_min
                           , data=dp_data, importance=T, proximity=T, do.trace=100,ntree=500)
...
#找出最適當的變量個數
ndp = cbind(dp[,4],dp[,5],dp[,6],dp[,7],dp[,8])
tuneRF(x=ndp, y=dp$hdf, mtryStart = 1, ntreeTry = 200)
...
########################### 針對hdf與單一欄位做預測 ###########################
randomforestM=randomForest(hdf~AirTemperature_K, data=dp_data, importance=T, proximity=T, do.trace=100,ntree=200)
