...
######################## 重新分類FailureType裡的值 #######################
dp = cbind(dp,FailureType_num = factor(
  ifelse(dp$FailureType == "Heat Dissipation Failure",1,
         ifelse(dp$FailureType == "Overstrain Failure",2,
                ifelse(dp$FailureType == "Power Failure",3,
                       ifelse(dp$FailureType == "Random Failures",4,
                              ifelse(dp$FailureType == "Tool Wear Failure",5,0)
...
table(dp$twf)
newData = smote(twf ~ ., dp, perc.over = 30,perc.under=3)
table(newData$twf)
